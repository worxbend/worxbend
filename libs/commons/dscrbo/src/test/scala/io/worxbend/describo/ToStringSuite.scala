...
class ToStringSuite extends AnyFunSuite:
  ...
  test(
    ...
    val expected =
      """|TestedType(
         ...
         |  accountBalance:::<BigDecimal> -- [1000.50],
         |  roles:::<$colon$colon> -- [List(Admin, User)],
         |  metadata:::<Map2> -- [Map(key1 -> value1, key2 -> value2)],
  ...
  test("should generate single string with redacted password and skipped username fields"):
    ...
    val expected =
      "TestedType(password = <redacted>, fullName = \"Test User\", age = 1, isAdmin = false, isSuperUser = true, isDisabled = false, createdDate = 2023-01-01, lastLogin = 2023-01-01T00:00:00Z, accountBalance = 1000.50, roles = List(Admin, User), metadata = Map(key1 -> value1, key2 -> value2), isVerified = Some(true), optionalComment = Some(This is a test comment))"
