...
object ToString {
      ...
      '{
        ...
        val skipOrReplaceInstructionsMap: Map[String, (Boolean, String)] =
          ($skipOrReplaceInstructExpr)
            ...
            .flatten
            .map {
              case (fieldName, shouldKeep, replacementValue) =>
                fieldName -> (shouldKeep, replacementValue)
            }
            .toMap
        ...
        val fieldsSep =
          if conf.multiline then conf.fieldsSeparator.toString + "\n"
            ...
            case (name, i) =>
              ...
              val fieldValue =
                replacementVal match {
                  case Some(replacementVal) =>
                    s"${conf.valuePrefix}$replacementVal${conf.valueSuffix}"
  ...
  inline def derived[T <: Product](
      inline x: T
  )(using inline configuration: Configuration = Configuration()): String = ${ toStringImpl[T]('x, 'configuration) }
