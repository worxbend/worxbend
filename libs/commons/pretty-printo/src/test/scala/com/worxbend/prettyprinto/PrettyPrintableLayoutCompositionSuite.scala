...
object LcId:
  ...
  given PrettyPrintable[LcId] = PrettyPrintable.valueClass[LcId, Long]("LcId", "com.worxbend.prettyprinto.LcId")(_.raw)
...
final class PrettyPrintableLayoutCompositionSuite extends AnyFunSuite:
  ...
  // The enum case renders as `c.w.p.Red` rather than `c.w.p.LcColour.Red`: Magnolia's TypeInfo reports the enclosing
  ...
  test("qualified names reach the field types and the enum case alike"):
    assert(
      ...
      ) == "c.w.p.LcTyped(t: s.Tuple2 = s.Tuple2(_1: s.Int = 1, _2: j.l.String = \"a\"), " +
        "e: c.w.p.LcColour = c.w.p.Red, id: c.w.p.LcId = 1, o: s.Option = None)"
    )
