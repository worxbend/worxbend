...
class PrettyPrintableConfigurationSuite extends AnyFunSuite:
  ...
  test("fullyQualifiedClassName = true with shortPackagePrefix = false prints the whole package"):
    ...
    assert(render(conf) == "com.worxbend.prettyprinto.ConfigPair(alpha = 1, beta = \"x\")")
  ...
  test("fullyQualifiedClassName = true with shortPackagePrefix = true compresses the package"):
    ...
    assert(render(conf) == "c.w.p.ConfigPair(alpha = 1, beta = \"x\")")
  ...
  test("fullyQualifiedClassName also applies to the declared field types"):
    ...
    assert(render(conf) == "com.worxbend.prettyprinto.ConfigPair(alpha: scala.Int = 1, beta: java.lang.String = \"x\")")
