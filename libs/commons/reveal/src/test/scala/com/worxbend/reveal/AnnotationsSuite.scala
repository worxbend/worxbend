...
final case class AnnNamedDefaultRedaction(@Redacted(replacement = "<redacted>") password: String)
    derives PrettyPrintable

final case class AnnExcluded(@Excluded hidden: String, name: String) derives PrettyPrintable

final case class AnnTransient(@transient hidden: String, name: String) derives PrettyPrintable
...
final case class AnnNestedSecret(@Redacted secret: String, tag: String) derives PrettyPrintable

final case class AnnHolder(inner: AnnNestedSecret, name: String) derives PrettyPrintable

final case class AnnListHolder(entries: List[AnnNestedSecret]) derives PrettyPrintable

final case class AnnOptionHolder(entry: Option[AnnNestedSecret]) derives PrettyPrintable

final case class AnnMapHolder(entries: Map[String, AnnNestedSecret]) derives PrettyPrintable
...
final case class AnnWallet(redacted: AnnRedactedValueClass, excluded: AnnExcludedValueClass) derives PrettyPrintable
...
final class AnnotationsSuite extends AnyFunSuite:
  ...
  test("@Redacted replaces the value with the default replacement"):
    assert(
      PrettyPrintable[AnnDefaultRedaction].describe(AnnDefaultRedaction("s3cret", "bob"))(using singleLine) ==
        "AnnDefaultRedaction(password = <redacted>, name = \"bob\")"
    )

  test("@Redacted honours a custom replacement"):
    assert(
      PrettyPrintable[AnnCustomRedaction].describe(AnnCustomRedaction("s3cret", "bob"))(using singleLine) ==
  ...
  test("@Redacted with an explicitly named default replacement behaves like the implicit one"):
    assert(
      PrettyPrintable[AnnNamedDefaultRedaction].describe(AnnNamedDefaultRedaction("s3cret"))(using singleLine) ==
        "AnnNamedDefaultRedaction(password = <redacted>)"
    )

  test("@Redacted never dereferences the value, so a null secret still renders"):
    assert(
      PrettyPrintable[AnnDefaultRedaction].describe(AnnDefaultRedaction(null, "bob"))(using singleLine) ==
        "AnnDefaultRedaction(password = <redacted>, name = \"bob\")"
    )

  test("@Redacted still reports the declared type under useTypeNames"):
    assert(
      PrettyPrintable[AnnDefaultRedaction].describe(AnnDefaultRedaction(null, "bob"))(using
        singleLine.copy(useTypeNames =
          true
  ...
  test("@Excluded omits the field entirely"):
    assert(
      PrettyPrintable[AnnExcluded].describe(AnnExcluded("hidden", "bob"))(using
        singleLine) == "AnnExcluded(name = \"bob\")"
    )

  test("@Excluded never dereferences the value, so a null field still renders"):
    assert(PrettyPrintable[AnnExcluded].describe(AnnExcluded(null, "bob"))(using
      singleLine) == "AnnExcluded(name = \"bob\")")

  test("@transient omits the field, exactly like @Excluded"):
    assert(
      PrettyPrintable[AnnTransient].describe(AnnTransient("hidden", "bob"))(using
        singleLine) == "AnnTransient(name = \"bob\")"
    )

  test("@Redacted before @Excluded is omitted, not redacted"):
    assert(
      PrettyPrintable[AnnRedactedThenExcluded].describe(AnnRedactedThenExcluded("s", "bob"))(using singleLine) ==
        "AnnRedactedThenExcluded(name = \"bob\")"
    )

  test("@Excluded before @Redacted is omitted as well, so source order is irrelevant"):
    assert(
      PrettyPrintable[AnnExcludedThenRedacted].describe(AnnExcludedThenRedacted("s", "bob"))(using singleLine) ==
        "AnnExcludedThenRedacted(name = \"bob\")"
    )

  test("repeated @Redacted takes the first annotation in declaration order"):
    assert(
      PrettyPrintable[AnnRepeatedRedaction].describe(AnnRepeatedRedaction("s"))(using singleLine) ==
        "AnnRepeatedRedaction(token = first)"
    )

  test("redaction composes through a nested case class"):
    assert(
      PrettyPrintable[AnnHolder].describe(AnnHolder(AnnNestedSecret("hunter2", "t"), "x"))(using singleLine) ==
        "AnnHolder(inner = AnnNestedSecret(secret = <redacted>, tag = \"t\"), name = \"x\")"
    )

  test("redaction composes through a list element"):
    assert(
      PrettyPrintable[AnnListHolder].describe(AnnListHolder(List(AnnNestedSecret("hunter2", "t"))))(using singleLine) ==
        "AnnListHolder(entries = [AnnNestedSecret(secret = <redacted>, tag = \"t\")])"
    )

  test("redaction composes through an Option payload"):
    assert(
      PrettyPrintable[AnnOptionHolder].describe(AnnOptionHolder(Some(AnnNestedSecret("hunter2", "t"))))(using
        singleLine) ==
  ...
  test("redaction composes through a map value"):
    assert(
      PrettyPrintable[AnnMapHolder].describe(AnnMapHolder(Map("k" -> AnnNestedSecret("hunter2", "t"))))(using
        singleLine) ==
  ...
  test("a redacted field is never dereferenced, even when its type could not be rendered at all"):
    assert(
      PrettyPrintable[AnnNeverDereferenced].describe(AnnNeverDereferenced(null, null, "bob"))(using singleLine) ==
  ...
  test("the same recursive type still derives when it is the root"):
    assertCompiles("PrettyPrintable.derived[AnnUnrenderable]")

  test("@Redacted on the sole parameter of a value class replaces the payload"):
    assert(
      PrettyPrintable[AnnWallet]
