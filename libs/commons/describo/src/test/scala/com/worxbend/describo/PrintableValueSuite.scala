...
final case class ValString(value: String) derives Printable

final case class ValChar(value: Char) derives Printable
...
final case class ValBoolean(value: Boolean) derives Printable
...
final case class ValStrings(values: List[String]) derives Printable

final case class ValBoxedInt(value: java.lang.Integer) derives Printable

final case class ValBoxedChar(value: java.lang.Character) derives Printable
...
class PrintableValueSuite extends AnyFunSuite:

  private def string(raw: String): String = summon[Printable[ValString]].asString(ValString(raw))
  ...
  test("a string element inside a collection is escaped too"):
    val actual = summon[Printable[ValStrings]].asString(ValStrings(List("a\"b")))
    assert(actual == "ValStrings(values = [\"a\\\"b\"])")

  test("a Char is wrapped in single quotes"):
    assert(summon[Printable[ValChar]].asString(ValChar('c')) == "ValChar(value = 'c')")

  test("a single quote Char is escaped"):
    assert(summon[Printable[ValChar]].asString(ValChar('\'')) == "ValChar(value = '\\'')")

  test("a newline Char is escaped"):
    assert(summon[Printable[ValChar]].asString(ValChar('\n')) == "ValChar(value = '\\n')")
  ...
  test("numeric values render without quotes"):
    ...
    assert(summon[Printable[ValNumbers]].asString(value)(using conf) == expected)
  ...
  test("numeric fields print their declared Scala types, not their boxed JVM classes"):
    ...
    val types = summon[Printable[ValNumbers]].asString(value)(using conf)
    assert(
  ...
  test("a Boolean renders unquoted"):
    assert(summon[Printable[ValBoolean]].asString(ValBoolean(true)) == "ValBoolean(value = true)")
  ...
  test("java.time values render through their own toString"):
    ...
    val conf     = Configuration(multilineIfFieldsAreGreaterOrEqual = 0)
    assert(summon[Printable[ValTemporal]].asString(value)(using conf) == expected)

  test("a boxed java.lang.Integer renders as its number"):
    assert(summon[Printable[ValBoxedInt]].asString(ValBoxedInt(7)) == "ValBoxedInt(value = 7)")

  test("a boxed java.lang.Integer field prints Integer as its declared type"):
    val conf = Configuration(useTypeNames = true)
    assert(summon[Printable[ValBoxedInt]].asString(ValBoxedInt(7))(using conf) == "ValBoxedInt(value: Integer = 7)")

  test("a boxed java.lang.Character is quoted exactly like a Char"):
    assert(summon[Printable[ValBoxedChar]].asString(ValBoxedChar('c')) == "ValBoxedChar(value = 'c')")

  test("a boxed java.lang.Character is escaped exactly like a Char"):
    assert(summon[Printable[ValBoxedChar]].asString(ValBoxedChar('\'')) == "ValBoxedChar(value = '\\'')")
  ...
  test("zoned and offset java.time values render through their own toString"):
    ...
    assert(summon[Printable[ValZoned]].asString(value) == expected)
  ...
  test("zoned and offset java.time fields print their declared types"):
    ...
    assert(summon[Printable[ValZoned]].asString(value)(using Configuration(useTypeNames = true)) == expected)
  ...
  test("java.time fields print their declared types"):
    ...
    val conf  = Configuration(useTypeNames = true, multilineIfFieldsAreGreaterOrEqual = 0)
    assert(summon[Printable[ValTemporal]].asString(value)(using conf).contains("date: LocalDate = 2023-01-01"))
